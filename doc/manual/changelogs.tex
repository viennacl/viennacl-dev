...
\begin{itemize}
 ...
 \item Vectors and matrices can be instantiated with integers. Operations so far only supported for signed integers.
 ...
 \item Added unary element-wise operations: \lstinline|element_sin()|, \lstinline|element_sqrt()|, etc.
 ...
 \item Added support for \lstinline|prod(A, B)|, where \lstinline|A| is either a \lstinline|compressed_matrix| or \lstinline|ell_matrix| and \lstinline|B| is a row-major dense matrix (kindly provided by Albert Zaharovits).
 ...
 \item Added possibility to switch command queues in OpenCL contexts (thanks to Evan Bollig).
 ...
 \item Completely eliminated the OpenCL kernel conversion step in the developer repository and the source-release. This also eliminates the need for Boost.
\end{itemize}
