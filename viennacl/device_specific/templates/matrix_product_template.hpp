...
private:
    ...
    std::string generate_impl(const std::string &kernel_prefix, const statements_container &statements, const std::vector<mapping_type> &mappings, bool fallback) const
    {
        ...
        std::string widthstr = tools::to_string(p.simd_width);
        ...
        switch(p.A_fetching_policy)
        {
          case FETCH_FROM_LOCAL:
            if(A_trans_=='N')
                stream << A->process("#pointer += gidx*" + to_string(p.mL) + "+ idxT*" + widthstr + " + idyT*#ld;") << std::endl;
            else
                stream << A->process("#pointer += gidx*" + to_string(p.mL) + "*#ld + idxT + idyT*#ld;") << std::endl;
            break;
        ...
        switch(p.B_fetching_policy)
        {
          case FETCH_FROM_LOCAL:
            if(B_trans_=='T')
                stream << B->process("#pointer += gidy*" + to_string(p.nL) + "+ idxT*" + widthstr + " + idyT*#ld;") << std::endl;
            else
                stream << B->process("#pointer += gidy*" + to_string(p.nL) + "*#ld + idxT + idyT*#ld;") << std::endl;
            break;
        ...
        if(p.A_fetching_policy==FETCH_FROM_LOCAL)
        {
            unsigned int bound1 = (A_trans_=='N')?p.kL:p.mL;
            unsigned int bound0 = (A_trans_=='N')?p.mL:p.kL;
            for(unsigned int k = 0 ; k < bound1 ; k += p.local_fetch_1)
                for(unsigned int m = 0 ; m < bound0 ; m += p.local_fetch_0*p.simd_width)
                {
                    unsigned int offset = (A_trans_=='N')?(k*(p.mL+1)+m):(m*(p.mL+1)+k);
                    stream << A->process(vstore + "(" + vload + "(" + to_string(m/p.simd_width) + ", #pointer + " + to_string(k) + "*#ld), 0, plA + " + to_string(offset) + ");") << std::endl;
                }
        }
        ...
        if(p.B_fetching_policy==FETCH_FROM_LOCAL)
        {
            unsigned int bound1 = (B_trans_=='T')?p.kL:p.nL;
            unsigned int bound0 = (B_trans_=='T')?p.nL:p.kL;
            for(unsigned int k = 0 ; k < bound1 ; k += p.local_fetch_1)
                for(unsigned int n = 0 ; n < bound0 ; n += p.local_fetch_0*p.simd_width)
                {
                    unsigned int offset = (B_trans_=='T')?k*(p.nL+1) + n:n*(p.nL+1) + k;
                    stream << B->process(vstore + "(" + vload + "(" + to_string(n/p.simd_width) + ", #pointer + " + to_string(k) + "*#ld), 0, plB + " + to_string(offset) + ");") << std::endl;
                }
                      ...
                      T beta_value)
    {
        ...
        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, 0, lM, 0, lK, swap_A), create_slice<T>(ptr_matrix, Bcopy, 0, lK, 0, lN, swap_B), create_slice<T>(ptr_matrix, Ccopy, 0, lM, 0, lN, false), beta_value, false);
        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, 0, lM, lK, K, swap_A), create_slice<T>(ptr_matrix, Bcopy, lK, K, 0, lN, swap_B), create_slice<T>(ptr_matrix, Ccopy, 0, lM, 0, lN, false), (T)1, true);

        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, 0, lM, 0, lK, swap_A), create_slice<T>(ptr_matrix, Bcopy, 0, lK, lN, N, swap_B), create_slice<T>(ptr_matrix, Ccopy, 0, lM, lN, N, false), beta_value, true);
        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, 0, lM, lK, K, swap_A), create_slice<T>(ptr_matrix, Bcopy, lK, K, lN, N, swap_B), create_slice<T>(ptr_matrix, Ccopy, 0, lM, lN, N, false), (T)1, true);

        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, lM, M, 0, lK, swap_A), create_slice<T>(ptr_matrix, Bcopy, 0, lK, 0, lN, swap_B), create_slice<T>(ptr_matrix, Ccopy, lM, M, 0, lN, false), beta_value, true);
        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, lM, M, lK, K, swap_A), create_slice<T>(ptr_matrix, Bcopy, lK, K, 0, lN, swap_B), create_slice<T>(ptr_matrix, Ccopy, lM, M, 0, lN, false), (T)1, true);

        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, lM, M, 0, lK, swap_A), create_slice<T>(ptr_matrix, Bcopy, 0, lK, lN, N, swap_B), create_slice<T>(ptr_matrix, Ccopy, lM, M, lN, N, false), beta_value, true);
        enqueue_block(statement, kernels, A, B, C, beta, create_slice<T>(ptr_matrix, Acopy, lM, M, lK, K, swap_A), create_slice<T>(ptr_matrix, Bcopy, lK, K, lN, N, swap_B), create_slice<T>(ptr_matrix, Ccopy, lM, M, lN, N, false), (T)1, true);
    }
