...
namespace viennacl{
  ...
  namespace scheduler{
    ...
    namespace preset{
      ...
      template<typename T>
      statement mat_mat_prod(T alpha, viennacl::matrix_base<T> const * A, bool A_trans,
                             viennacl::matrix_base<T> const * B, bool B_trans,
                             T beta, viennacl::matrix_base<T> const * C)
      {
        vcl_size_t dummy = 0;
