...
namespace viennacl
{
  namespace scheduler
  {
    ...
    inline void execute_matrix_row_inplace_add_composite(statement const & s, statement_node const & root_node)
    {
    ...
    inline void execute_matrix_row_inplace_add_matrix(statement const & s, statement_node const & root_node)
    {
    ...
    inline void execute_matrix_row_inplace_add(statement const & s, statement_node const & root_node)
    {
      typedef statement::container_type   StatementContainer;
      ...
      switch (expr[0].rhs_type_family)
      {
        case COMPOSITE_OPERATION_FAMILY:
          execute_matrix_row_inplace_add_composite(s, root_node);
          break;
        case MATRIX_ROW_TYPE_FAMILY:
          execute_matrix_row_inplace_add_matrix(s, root_node);
          break;
        default:
