...
namespace viennacl
{
  namespace scheduler
  {
    ...
    void execute_matrix_col_assign_composite(statement const & s)
    {
      typename statement::container_type const & expr = s.array();

      if (expr[1].op_type_  == OPERATION_BINARY_ADD_TYPE)
      {
        if (expr[0].lhs_type_ == MATRIX_COL_FLOAT_TYPE
            && expr[1].lhs_type_ == MATRIX_COL_FLOAT_TYPE
            && expr[1].rhs_type_ == MATRIX_COL_FLOAT_TYPE)
        {
          viennacl::matrix_base<float, viennacl::column_major>       & A = *(expr[0].lhs_.matrix_col_float_);
          viennacl::matrix_base<float, viennacl::column_major> const & B = *(expr[1].lhs_.matrix_col_float_);
          viennacl::matrix_base<float, viennacl::column_major> const & C = *(expr[1].rhs_.matrix_col_float_);
          viennacl::linalg::ambm(A,
                                 B, 1.0, 1, false, false,
                                 C, 1.0, 1, false, false);
        }
        else if (expr[0].lhs_type_ == MATRIX_COL_DOUBLE_TYPE
                 && expr[1].lhs_type_ == MATRIX_COL_DOUBLE_TYPE
                 && expr[1].rhs_type_ == MATRIX_COL_DOUBLE_TYPE)
        {
          viennacl::matrix_base<double, viennacl::column_major>       & A = *(expr[0].lhs_.matrix_col_double_);
          viennacl::matrix_base<double, viennacl::column_major> const & B = *(expr[1].lhs_.matrix_col_double_);
          viennacl::matrix_base<double, viennacl::column_major> const & C = *(expr[1].rhs_.matrix_col_double_);
          viennacl::linalg::ambm(A,
                                 B, 1.0, 1, false, false,
                                 C, 1.0, 1, false, false);
        }
    ...
    void execute_matrix_col_assign_matrix(statement const & s)
    {
    ...
    void execute_matrix_col_assign(statement const & s)
    {
      typedef typename statement::container_type   StatementContainer;
