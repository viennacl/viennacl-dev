...
namespace viennacl
{
  namespace traits
  {
    ...
    template <typename T>
    viennacl::ocl::handle<cl_mem> handle(viennacl::scalar_expression< const scalar<T>, const scalar<T>, op_flip_sign> const & obj)
    {
      return viennacl::ocl::handle<cl_mem>(obj.get_lhs().handle());
    }
