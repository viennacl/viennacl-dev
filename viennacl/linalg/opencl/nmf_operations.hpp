    ...
    namespace opencl
    {
          ...
          viennacl::matrix_base<ScalarType> & H, viennacl::linalg::nmf_config const & conf)
      {
        viennacl::ocl::context & ctx =
            const_cast<viennacl::ocl::context &>(viennacl::traits::opencl_handle(V).context());
        ...
        viennacl::matrix_base<ScalarType> wn(V.size1(), k, W.row_major(), ctx);
        viennacl::matrix_base<ScalarType> wd(V.size1(), k, W.row_major(), ctx);
        viennacl::matrix_base<ScalarType> wtmp(V.size1(), V.size2(), W.row_major(), ctx);

        viennacl::matrix_base<ScalarType> hn(k, V.size2(), H.row_major(), ctx);
        viennacl::matrix_base<ScalarType> hd(k, V.size2(), H.row_major(), ctx);
        viennacl::matrix_base<ScalarType> htmp(k, k, H.row_major(), ctx);

        viennacl::matrix_base<ScalarType> appr(V.size1(), V.size2(), V.row_major(), ctx);
        ...
        for (vcl_size_t i = 0; i < conf.max_iterations(); i++)
        {
          conf.iters_ = i + 1;
          {
            ...
            viennacl::ocl::kernel & mul_div_kernel = ctx.get_kernel(
                viennacl::linalg::opencl::kernels::nmf<ScalarType>::program_name(),
                NMF_MUL_DIV_KERNEL);
            viennacl::ocl::enqueue(
                mul_div_kernel(H, hn, hd, cl_uint(H.internal_size1() * H.internal_size2())));
          }
          {
            ...
            viennacl::ocl::enqueue(
                mul_div_kernel(W, wn, wd, cl_uint(W.internal_size1() * W.internal_size2())));
          }
