...
namespace detail
{
            ...
            NumericT alpha, NumericT beta)
  {
    ...
    vcl_size_t num_blocks_A2 = ((A_trans ? A_size1 : A_size2) - 1) / blocksize + 1;

    //std::cout << "num_blocksA2 " << num_blocks_A2 << std::endl;//DEBUG	
    ...
    for (long block_idx_i2=0; block_idx_i2<static_cast<long>(num_blocks_C1); ++block_idx_i2)
    {
      ...
      for (vcl_size_t block_idx_j=0; block_idx_j<num_blocks_C2; ++block_idx_j)
      {
        ...
        for (vcl_size_t block_idx_k=0; block_idx_k<num_blocks_A2; ++block_idx_k)
        {
          ...
          pack_matrix_in_row_major(buffer_A, offset_i, offset_k, blocksize, data_A, A_size1, A_size2, A_internal_size1, A_internal_size2, A_inc1, A_inc2, A_start1, A_start2,  A_trans, A_row_major);

          /*          for (vcl_size_t j = offset_j; j < std::min(offset_j + blocksize, C_size2); ++j)
            for (vcl_size_t k = offset_k; k < std::min(offset_k + blocksize, A_size2); ++k)
            buffer_B[(k - offset_k) + (j - offset_j) * blocksize] = B(k, j);*/
          pack_matrix_in_col_major(buffer_B, offset_k, offset_j, blocksize, data_B, B_size1, B_size2, B_internal_size1, B_internal_size2, B_inc1, B_inc2, B_start1, B_start2,  B_trans, B_row_major);
               ...
               ScalarT2 beta)
{
  ...
  //std::cout << "A[1] is " << viennacl::linalg::host_based::detail::extract_raw_pointer<NumericT>(A)[1] << std::endl;//DEBUG 
  //std::cout << "A is row-major? " << A.row_major() << std::endl;//DEBUG
  //std::cout << std::endl <<"    -> A sizes: " << A.size1() << " " << A.size2() << " " << A.internal_size1() << " " << A.internal_size2() << std::endl;//DEBUG
  //std::cout << "    -> B sizes: " << B.size1() << " " << B.size2() << " " << B.internal_size1() << " " << B.internal_size2() << std::endl;//DEBUG
  ...
  */
  ...
  if (!trans_A && !trans_B)
  {
    if (A.row_major() && B.row_major() && C.row_major())
    {
      ...
      detail::prod(A, trans_A, B, trans_B, wrapper_C, C_size1, C_size2, A_size2, static_cast<value_type>(alpha), static_cast<value_type>(beta));
    }
  ...
  else if (trans_A && !trans_B)
  {
    if (A.row_major() && B.row_major() && C.row_major())
    {
      ...
      detail::prod(A, trans_A, B, trans_B, wrapper_C, C_size1, C_size2, A_size1, static_cast<value_type>(alpha), static_cast<value_type>(beta));
    }
    else if (A.row_major() && B.row_major() && !C.row_major())
    {
      //detail::matrix_array_wrapper<value_type const, row_major, true>    wrapper_A(data_A, A_start1, A_start2, A_inc1, A_inc2, A_internal_size1, A_internal_size2);
