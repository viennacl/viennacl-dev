...
namespace viennacl
{
  namespace linalg
  {
    ...
    {
      /** @brief Main OpenMP kernel for nonnegative matrix factorization of a dense matrices. */
      template<typename StringType>
      void el_wise_mul_div(StringType * matrix1, const StringType * matrix2,
          const StringType * matrix3, unsigned int size)
      {
...
#pragma omp parallel for
        ...
        {
          StringType val = matrix1[i] * matrix2[i];
          StringType divisor = matrix3[i];
          matrix1[i] = (divisor > (StringType) 0.00001) ? (val / divisor) : (StringType) 0;
        }
      }
          ...
          viennacl::matrix_base<ScalarType> & H, viennacl::linalg::nmf_config const & conf)
      {
        ...
        if (!viennacl::linalg::norm_frobenius(W))
          W = viennacl::scalar_matrix<ScalarType>(W.size1(), W.size2(), (ScalarType) 1.0);

        if (!viennacl::linalg::norm_frobenius(H))
          H = viennacl::scalar_matrix<ScalarType>(H.size1(), H.size2(), (ScalarType) 1.0);

        viennacl::matrix_base<ScalarType> wn(V.size1(), k, W.row_major());
        viennacl::matrix_base<ScalarType> wd(V.size1(), k, W.row_major());
        viennacl::matrix_base<ScalarType> wtmp(V.size1(), V.size2(), W.row_major());

        viennacl::matrix_base<ScalarType> hn(k, V.size2(), H.row_major());
        viennacl::matrix_base<ScalarType> hd(k, V.size2(), H.row_major());
        viennacl::matrix_base<ScalarType> htmp(k, k, H.row_major());

        viennacl::matrix_base<ScalarType> appr(V.size1(), V.size2(), V.row_major());
        ...
        for (vcl_size_t i = 0; i < conf.max_iterations(); i++)
        {
          ...
          ScalarType * data_H = detail::extract_raw_pointer<ScalarType>(H);
          ScalarType * data_hn = detail::extract_raw_pointer<ScalarType>(hn);
          ScalarType * data_hd = detail::extract_raw_pointer<ScalarType>(hd);

          viennacl::linalg::host_based::el_wise_mul_div(data_H, data_hn, data_hd,
              H.internal_size1() * H.internal_size2());

          wn = viennacl::linalg::prod(V, trans(H));
          wtmp = viennacl::linalg::prod(W, H);
          wd = viennacl::linalg::prod(wtmp, trans(H));

          ScalarType * data_W = detail::extract_raw_pointer<ScalarType>(W);
          ScalarType * data_wn = detail::extract_raw_pointer<ScalarType>(wn);
          ScalarType * data_wd = detail::extract_raw_pointer<ScalarType>(wd);

          viennacl::linalg::host_based::el_wise_mul_div(data_W, data_wn, data_wd,
