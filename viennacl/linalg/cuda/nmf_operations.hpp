...
namespace viennacl
{
  namespace linalg
  {
    namespace cuda
    {
      ...
      template<typename T>
      __global__ void el_wise_mul_div(
          T * matrix1,
          const T * matrix2,
          const T * matrix3,
          unsigned int size)
      {
        ...
        {
          T val = matrix1[i] * matrix2[i];
          T divisor = matrix3[i];
          matrix1[i] = (divisor > (T) 0.00001) ? (val / divisor) : (T) 0;
        }
      }
          ...
          viennacl::matrix_base<ScalarType> & H, viennacl::linalg::nmf_config const & conf)
      {
        ...
        for (vcl_size_t i = 0; i < conf.max_iterations(); i++)
        {
          ...
          el_wise_mul_div<<<128, 128>>>(
            detail::cuda_arg<ScalarType>(H),
            detail::cuda_arg<ScalarType>(hn),
            detail::cuda_arg<ScalarType>(hd),
          ...
          el_wise_mul_div<<<128, 128>>>(
              detail::cuda_arg<ScalarType>(W),
              detail::cuda_arg<ScalarType>(wn),
              detail::cuda_arg<ScalarType>(wd),
