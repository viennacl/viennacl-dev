    ...
    namespace single_threaded
    {
      ...
      */
                           ...
                           viennacl::vector<ScalarType, VECTOR_ALIGNMENT> & result)
      {
        ...
        cl_uint    const * row_buffer = detail::extract_raw_pointer<cl_uint>(mat.handle1());    //TODO: Remove cl_uint dependency
        cl_uint    const * col_buffer = detail::extract_raw_pointer<cl_uint>(mat.handle2());    //TODO: Remove cl_uint dependency
      ...
      */
                           ...
                           viennacl::vector<ScalarType, VECTOR_ALIGNMENT> & result)
      {
        ...
        ScalarType const * vec_buf      = detail::extract_raw_pointer<ScalarType>(vec.handle());
        ScalarType const * elements     = detail::extract_raw_pointer<ScalarType>(mat.handle());
        cl_uint    const * coord_buffer = detail::extract_raw_pointer<cl_uint>(mat.handle12());    //TODO: Remove cl_uint dependency
      ...
      */
                           ...
                           viennacl::vector<ScalarType, VECTOR_ALIGNMENT> & result)
      {
        ...
        ScalarType const * vec_buf      = detail::extract_raw_pointer<ScalarType>(vec.handle());
        ScalarType const * elements     = detail::extract_raw_pointer<ScalarType>(mat.handle());
        cl_uint    const * coords       = detail::extract_raw_pointer<cl_uint>(mat.handle2());    //TODO: Remove cl_uint dependency
        ...
        for(std::size_t row = 0; row < mat.size1(); ++row)
        {
          ...
          for(uint item_id = 0; item_id < mat.internal_maxnnz(); ++item_id)
          {
      ...
      */
                           ...
                           viennacl::vector<ScalarType, VECTOR_ALIGNMENT> & result)
      {
        ...
        cl_uint    const * csr_row_buffer = detail::extract_raw_pointer<cl_uint>(mat.handle3());    //TODO: Remove cl_uint dependency
        cl_uint    const * csr_col_buffer = detail::extract_raw_pointer<cl_uint>(mat.handle4());    //TODO: Remove cl_uint dependency
        ...
        for(std::size_t row = 0; row < mat.size1(); ++row)
        {
          ...
          //
          for(uint item_id = 0; item_id < mat.internal_ellnnz(); ++item_id)
          {
            std::size_t offset = row + item_id * mat.internal_size2();
            ScalarType val = elements[offset];
