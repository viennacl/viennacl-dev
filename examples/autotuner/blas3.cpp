...
void run_autotune(std::string const & dump_name, bool is_lhs_trans, bool is_rhs_trans){
    ...
    for(std::list<std::pair<unsigned int, unsigned int> >::iterator it = rounds_config.begin() ; it!= rounds_config.end(); ++it){
        ...
        profiles::key_type keyAA(MATRIX_PRODUCT_AA_TYPE, scalartype_size);
        profiles::key_type keyTA(MATRIX_PRODUCT_TA_TYPE, scalartype_size);
        profiles::key_type keyAT(MATRIX_PRODUCT_AT_TYPE, scalartype_size);
        profiles::key_type keyTT(MATRIX_PRODUCT_AA_TYPE, scalartype_size);
