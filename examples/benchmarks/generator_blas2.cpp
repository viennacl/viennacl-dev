...
double run_benchmark(size_t matrix_size)
{
    ...
    std::vector<ScalarType> stl_C(matrix_size);
    ...
    for (unsigned int i = 0; i < matrix_size; ++i)
         stl_C[matrix_size] = random<ScalarType>();
    ...
    viennacl::vector<ScalarType> vcl_A(matrix_size);
    viennacl::matrix<ScalarType,FB> vcl_B(matrix_size, matrix_size);
    viennacl::vector<ScalarType> vcl_C(matrix_size);

    typedef viennacl::generator::dummy_vector< ScalarType > dva_t;
    typedef viennacl::generator::dummy_matrix< viennacl::matrix<ScalarType,FB> > dmb_t;
    typedef viennacl::generator::dummy_vector< ScalarType> dvc_t;
        ...
        for(devices_type::iterator it = dev.begin() ; it != dev.end() ; ++it){
            ...
            if(current_device++ == requested_device ){
                ...
                for(unsigned int size = SIZE_INC ; size <= MAX_SIZE ; size += SIZE_INC){
                    ...
                    if(scalartype=="float"){
                        switch(layout){
                        case 0 : exec_time = run_benchmark<float,viennacl::row_major>(size); break;
                        case 1 : exec_time = run_benchmark<float,viennacl::column_major>(size); break;
                        }
                    }
                    else if(scalartype=="double"){
                        switch(layout){
                        case 0 : exec_time = run_benchmark<double,viennacl::row_major>(size); break;
                        case 1 : exec_time = run_benchmark<double,viennacl::column_major>(size); break;
                        }
                    }
                    else{
